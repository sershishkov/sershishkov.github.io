...
///////////////////////////////////////////////////////////////////
    //1. Массив скиллов (поле skills) всех людей, не должно быть
    // повторяющихся скиллов, так же они должны быть отсортированы по
    // алфавиту;
    ...
    var sort =  _.sortBy(uniq, function (item) {
        return item;
    });
    console.log(sort);
    ...
    var sortName =  _.sortBy(allNames, function (item) {
        return item.friends;
    });
    console.log(sortName);
    //3. Массив всех друзей всех пользователей, не должно
    // быть повторяющихся людей
    var allFriends =_.map(myData, function(item) {

        var ff =_.map(item.friends, function(itemfr) {
            return itemfr.name;
        });
        return ff;
    });
    var flattFR = _.flatten(allFriends);
    var uniqFR=_.union(flattFR);
    console.log(uniqFR);
});